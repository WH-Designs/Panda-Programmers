...
namespace UnitTests
{
    public enum DbPersistence
    {
        OneDbPerTest,
        ReuseDb
    }

    public class InMemoryDbHelper<TContext> : IDisposable
        where TContext : DbContext
    {
        ...
        private void Initialize()
        {
            ...
            using TContext context = (TContext)
                Activator.CreateInstance(typeof(TContext), _dbContextOptions); // new TContext(_dbContextOptions);
            if (!context.Database.EnsureCreated())
            {
                Debug.WriteLine(
                    "Entity Framework could not ensure the db was created using Code First"
                );
            }
            // Seed data
